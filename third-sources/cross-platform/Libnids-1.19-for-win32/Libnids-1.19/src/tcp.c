...
add_new_tcp(struct tcphdr * this_tcphdr, struct ip * this_iphdr)
{
  ...
  printf("add_new_tcp src:%d.%d.%d.%d\n", (addr.saddr & 0xff), ((addr.saddr >> 8) & 0xff), ((addr.saddr >> 16) & 0xff), (addr.saddr >> 24));
  printf("add_new_tcp dst:%d.%d.%d.%d\n", (addr.daddr & 0xff), ((addr.daddr >> 8) & 0xff), ((addr.daddr >> 16) & 0xff), (addr.daddr >> 24));
  ...
  printf("add_new_tcp hash index:%d, tcp ptr:%p\n", hash_index, a_tcp);
...
process_tcp(u_char * data, int skblen)
{
  ...
  printf("process_tcp check1\n");
  if (datalen < 0) {
    nids_params.syslog(NIDS_WARN_TCP, NIDS_WARN_TCP_HDR, this_iphdr,
		       this_tcphdr);
    return;
  } // ktos sie bawi
  printf("process_tcp check2\n");
  unsigned long srclng_addr = this_iphdr->ip_src.s_addr;
  unsigned long dstlng_addr = this_iphdr->ip_dst.s_addr;
  printf("process_tcp src:%d.%d.%d.%d\n", (srclng_addr & 0xff), ((srclng_addr >> 8) & 0xff), ((srclng_addr >> 16) & 0xff), (srclng_addr >> 24));
  printf("process_tcp dst:%d.%d.%d.%d\n", (dstlng_addr & 0xff), ((dstlng_addr >> 8) & 0xff), ((dstlng_addr >> 16) & 0xff), (dstlng_addr >> 24));
  ...
  printf("process_tcp check3\n");
  if (!nids_params.n_tcp_streams) return;
  printf("process_tcp check4\n");
  ...
  printf("process_tcp check5\n");
  check_flags(this_iphdr, this_tcphdr);
  if (!(a_tcp = find_stream(this_tcphdr, this_iphdr, &from_client))) {
	  printf("s:%d, d:%d, seq:%d, flag:%d\n", (unsigned int)ntohs(this_tcphdr->th_sport), (unsigned int)ntohs(this_tcphdr->th_dport), (unsigned int)this_tcphdr->th_seq, (unsigned int)this_tcphdr->th_flags);
  ...
  printf("process_tcp check9\n");
  /* PAWS check */
  if (rcv->ts_on && get_ts(this_tcphdr, &tmp_ts) && 
  	before(tmp_ts, snd->curr_ts))
  return; 	
  printf("process_tcp 2\n");
  if ((this_tcphdr->th_flags & TH_ACK)) {
	  printf("process_tcp 3\n");
    if (from_client && a_tcp->client.state == TCP_SYN_SENT &&
	a_tcp->server.state == TCP_SYN_RECV) {
		printf("process_tcp 4\n");
	...
	a_tcp->client.ack_seq = ntohl(this_tcphdr->th_ack);
	{
	  ...
	  printf("process_tcp for tcp_procs \n");
