...
SOCKET mcast_send_socket(char* localIP, char* multicastIP, char* multicastPort,  int multicastTTL, struct addrinfo **multicastAddr) {
    ...
    fprintf(stdout, "running env checked: windows\n");
#endif
    ...
    */
	...
	addr.sin_port = htons(atoi(multicastPort)); //MULTICAST_GROUP_PORT
    ...
    */
	...
	//in_addr_t iface;// = INADDR_ANY; /* well, yeah, any */
	 ...
	 //iface.s_addr=inet_addr("192.168.1.129");
	 struct sockaddr_in ifacesockaa;
	 memset(&ifacesockaa, 0, sizeof(ifacesockaa));
	 ifacesockaa.sin_addr.s_addr = inet_addr(localIP);
	if(setsockopt (sock, 
		       IPPROTO_IP,
		       IP_MULTICAST_IF,
		       (char*)&ifacesockaa.sin_addr, sizeof(struct in_addr)) != 0) { 
