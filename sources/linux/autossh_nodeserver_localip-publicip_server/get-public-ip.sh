...
do
sleep $mcycle
localip=`ifconfig | grep 192.168 | awk -F' ' '{print $2}'`
mpip=`$icmd`
mdate=`date`
echo "" > $mfile
echo "$icmd" >> $mfile
...
echo $mpip >> $mfile
echo $localip >> $mfile
curl -k -v -X POST -d "{\"ip\":\"$mpip : $localip\",\"date\":\"$mdate\",\"cmd\": \"$icmd\"}" http://cf.beidouapp.com:20038/api/ips  --header "Content-Type:application/json"
done
done
